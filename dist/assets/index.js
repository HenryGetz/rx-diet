...
export function extractAssets(data) {
    ...
    if (cleaned.picture) {
        count += checkAndReplace(cleaned, 'picture.url', cleaned.picture, 'url', store);
    }
    if (cleaned.basics?.customFields && Array.isArray(cleaned.basics.customFields)) {
        for (let i = 0; i < cleaned.basics.customFields.length; i++) {
            ...
            if (!field)
                continue;
            count += checkAndReplace(cleaned, `basics.customFields.${i}.icon`, field, 'icon', store);
        }
    }
    if (cleaned.sections) {
        for (const sectionKey of Object.keys(cleaned.sections)) {
 ...
 */
export function restoreAssets(data, store) {
    walkAndRestore(data, store);
}
/**
 * Read an asset sidecar JSON file.
 */
export async function readAssetStore(path) {
 ...
 */
export async function writeAssetStore(path, store) {
 ...
 */
function checkAndReplace(_cleaned, path, obj, key, store) {
    const value = obj[key];
 ...
 */
function walkAndRestore(obj, store) {
 ...
 */
function lookupSentinel(str, store) {
    if (!str.startsWith(ASSET_SENTINEL_PREFIX))
        return undefined;
