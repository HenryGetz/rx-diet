...
export function parseFrontmatter(markdown) {
    if (!markdown.startsWith(FM_DELIMITER)) {
        throw new Error('No YAML frontmatter found. Expected content to start with "---\\n" followed by YAML and "---\\n".');
    }
    const closingIndex = markdown.indexOf('\n---', FM_DELIMITER.length);
    if (closingIndex === -1) {
        throw new Error('Unterminated YAML frontmatter. Expected closing "---" delimiter after frontmatter content.');
    }
    ...
    try {
        ...
        throw new Error(`Failed to parse YAML frontmatter: ${cause instanceof Error ? cause.message : String(cause)}`);
    }
    if (typeof parsed !== 'object' || parsed === null) {
        throw new Error('YAML frontmatter did not produce an object. Expected a mapping of key-value pairs.');
    }
    return [parsed, body];
}
/**
...
export function serializeFrontmatter(fm) {
    ...
    try {
        ...
        throw new Error(`Failed to serialize frontmatter to YAML: ${cause instanceof Error ? cause.message : String(cause)}`);
    }
...
function collectSectionIds(sectionKey, items, map) {
    if (!items)
        return;
 ...
 */
export function buildIdMap(data) {
    const map = {};
    ...
    for (const key of sectionKeys) {
        ...
        if (section && 'items' in section && Array.isArray(section.items)) {
            collectSectionIds(key, section.items, map);
        }
    }
