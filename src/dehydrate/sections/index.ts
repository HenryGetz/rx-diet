...
function stub(_data: unknown): string {
  return '';
}
 ...
 */
function wrap<T>(fn: (data: T) => string): (data: unknown) => string {
  return (data: unknown) => fn(data as T);
}

export const SECTION_SERIALIZERS: Record<string, (data: unknown) => string> = {
  // Implemented
