...
import type { PublicationsSection } from '../../utils/types.js';

export function serializePublications(section: PublicationsSection): string {
  if (section.items.length === 0) return '';

  const lines: string[] = [];
  lines.push(`## ${section.title || 'Publications'}`);
  lines.push('');
  ...
  for (const item of section.items) {
    ...
    const headingParts = [item.title, item.publisher, date].filter(
      p => (p ?? '').trim().length > 0,
    );
