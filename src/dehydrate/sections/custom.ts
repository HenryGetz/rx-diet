 ...
 */
function getHeadingName(item: Record<string, unknown>, index: number): string {
 ...
 */
export function serializeCustom(section: CustomSection): string {
  ...
  for (let i = 0; i < visible.length; i++) {
    ...
    let headingFieldName: string | null = null;
    for (const [key, value] of Object.entries(item)) {
      if (EXCLUDED_KEYS.has(key)) continue;
    ...
    for (const [key, value] of Object.entries(item)) {
      if (EXCLUDED_KEYS.has(key)) continue;
      if (key === headingFieldName) continue;
