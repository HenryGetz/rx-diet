...
import type { CertificationsSection } from '../../utils/types.js';

export function serializeCertifications(section: CertificationsSection): string {
  ...
  for (const item of section.items) {
    ...
    const headingParts = [item.title, item.issuer, date].filter(
      p => (p ?? '').trim().length > 0,
    );
