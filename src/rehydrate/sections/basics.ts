import type { Basics, Website, CustomField } from '../../utils/types.js';
 ...
 */
export function normalizeBasics(raw: Record<string, unknown>): Basics {
  ...
  if (Array.isArray(rawCustom)) {
    for (let i = 0; i < rawCustom.length; i++) {
      const item = rawCustom[i] as Record<string, string> | undefined;
