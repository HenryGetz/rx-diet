 ...
 */
export function mergeEntry<T extends Record<string, unknown>>(
  baseItem: T,
  markdownFields: Record<string, unknown>,
  sectionType: string,
  resolution: IdentityResolution,
): T {
  // New entry — return markdown fields as the item
  if (!resolution.resolved) {
    return markdownFields as unknown as T;
  }
  ...
  for (const field of fields) {
    if (field in markdownFields && markdownFields[field] !== undefined) {
      (result as Record<string, unknown>)[field] = markdownFields[field];
    }
  }
