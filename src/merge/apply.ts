import type { ResumeData, SectionName } from '../utils/types.js';
...
export function applyMerge(
  ...
  const merged: ResumeData = JSON.parse(JSON.stringify(base));
  ...
  for (const sectionName of SECTION_NAMES) {
    ...
    for (const entry of parsedEntries) {
      if (entry.resolution.resolved) {
        ...
        if (baseItem) {
          ...
          warnings.push(
            `Entry resolved to id "${resolvedId}" in section "${sectionName}" but no matching base item found — adding as new entry`,
          );
        ...
        const newItem: Record<string, unknown> = { ...entry.fields };
        mergedItems.push(newItem);
        changes.push({
          section: sectionName,
          index: mergedItems.length - 1,
    ...
    (merged.sections[sectionName] as unknown as { items: Record<string, unknown>[] }).items = mergedItems;
  }
