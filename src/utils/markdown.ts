...
export function htmlToMarkdown(html: string): string {
  ...
  const exoticTagRegex =
    /<(div|span|table|thead|tbody|tr|th|td|img|video|audio|canvas|svg|figure|figcaption|details|summary|nav|header|footer|section|article|aside)\b[^>]*>/i;
  const styledTagRegex =
    /<[a-z][a-z0-9]*\b[^>]*(?:style|class|data-)\s*=/i;
