...
export async function lintRxResumeMd(path: string): Promise<{ errors: LintError[] }> {
  ...
  let parsed: { basics: Record<string, unknown>; sections: Array<{ name: string; heading: string; entries: Array<unknown>; isCustom: boolean }> };
  try {
  ...
  for (const section of parsed.sections) {
    ...
    if (listBasedSections.has(section.name)) continue;
    for (const entry of section.entries) {
      const e = entry as { heading?: string; identityComment?: string | null };
